...
import { addCustomEdge, updateCustomEdge } from '../../db/customContent';
...
interface Props {
  ...
  editing: CustomEdge | null;
  /** Колбэк после сохранения/отмены. */
  onSaved: () => void;
  onCancel: () => void;
}

/**
 * Форма создания/редактирования кастомной черты.
 */
export function EdgeForm({ editing, onSaved, onCancel }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        await updateCustomEdge(editing.id, {
          name: name.trim(),
      ...
      console.error(err);
      alert('Не удалось сохранить черту.');
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="panel" style={{ marginTop: '16px' }}>
      <h3 style={{ marginTop: 0, marginBottom: '16px', fontSize: '16px' }}>
        {editing ? 'Редактировать черту' : 'Новая черта'}
      </h3>
      ...
      <input
        ...
        placeholder="Например, Мастер клинка"
        className="input"
      />
      ...
      <textarea
        ...
        placeholder="Что делает эта черта?"
        className="input"
        rows={3}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        <div>
          <div className="tiny" style={{ marginBottom: '4px' }}>Минимальный ранг</div>
          <select
            value={minRank}
